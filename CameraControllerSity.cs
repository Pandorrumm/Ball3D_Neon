...
public class CameraControllerSity : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (player.gameObject.GetComponent<MovePlayerSity>().jump)
        {
        ...
        if (player.gameObject.GetComponent<MovePlayerSity>().highJump)
        {
            gameObject.transform.position = new Vector3((player.transform.position.x - offset.x) * 0.8f,
                                                     4.5f,
                                                     player.transform.position.z - offset.z);
